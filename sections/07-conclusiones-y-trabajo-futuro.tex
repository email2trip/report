...
La organización eficiente de la información vinculada a viajes suele ser un desafío, debido a que están
...
El presente proyecto se centró en la extracción de información relevante contenida exclusivamente en correos
...
contenidos en correos electrónicos de confirmación,
...
Con el uso de CoreNLP se pudo ver que el proceso de \textit{Named Entity Recognition} es solo el primer paso para una correcta
identificación.
...
electrónicos, similar a como se implementa un filtro de \textit{spam}.
El clasificador produciría un valor de confianza para decidir en tiempo real si el correo corresponde a una reserva de
vuelos, de trenes o de otro tipo, y así optar por el \textit{pipeline} apropiado cuando el usuario realiza
\textit{drag \& drop} de cualquier correo en una sola área.

También se podrían incorporar otros \textit{pipelines} que cubran dominios no alcanzados por la implementación actual,
como estadías en hoteles, alquileres de autos, atracciones y reservas en restaurantes.
También se puede examinar la implementación de un \textit{pipeline} en español para los dominios de vuelos y trenes.
Para cada uno de estos nuevos \textit{pipelines} se deberán abordar una serie de desafíos propios de cada dominio concreto.

Finalmente, respecto a la usabilidad, una mejora técnica posible es sustituir la tarea de exportar un correo a
\texttt{.eml} para que, en cambio, el usuario reenvíe los correos a una casilla como \texttt{trips@email2trip.xyz}, de
forma que la aplicación analice los correos recibidos y los asigne según el remitente, que debe estar registrado en la
aplicación con esa misma dirección de correo.
